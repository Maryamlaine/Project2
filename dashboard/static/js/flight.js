const urls = {
    ...
    map: "static/data/states-albers-10m.json",
  
   
    // airports: "postgresql://postgres:postgres@flightdb.cbg99jbqtg8u.us-east-2.rds.amazonaws.com",
    airport_url: "/airports",
    flight_url :"/flights"
  
    // flights: "postgresql://postgres:postgres@flightdb.cbg99jbqtg8u.us-east-2.rds.amazonaws.com"
    };
...
function updateChart() {
    ...
    var dropdownMenu = d3.select("#year");
  ...
  const promises = [
   d3.json(urls.airport_url, typeAirport),
   d3.json(urls.flight_url + "/" + year,  typeFlight)
  ];
  ...
  function processData(values) {
    ...
    airports = airports.map (airport =>typeAirport(airport));
    flight = flights.map (flight =>typeFlight(flight));
    ...
    //Find data
    flights = flights.filter(link => iata.has(link.origin) && iata.has(link.destination));
    ...
    flights.forEach(function (link) {
      ...
      link.target = iata.get(link.destination);
        if (iata.get(link.origin) && iata.get(link.destination)) {
        if (isNaN(link.source.outgoing)) {
          link.source.outgoing = link.count
        }
        else {
          link.source.outgoing += link.count;
        }
        if (isNaN(link.source.incoming)) {
          link.source.incoming = link.count;
        }
        else {
